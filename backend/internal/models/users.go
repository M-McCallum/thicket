...
func (q *Queries) CreateUser(ctx context.Context, arg CreateUserParams) (User, error) {
	row := q.db.QueryRow(ctx,
		...
		VALUES ($1, $2, $3, $4)
		RETURNING id, username, email, avatar_url, display_name, status, kratos_id, created_at, updated_at`,
...
func (q *Queries) GetUserByID(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRow(ctx,
		`SELECT id, username, email, avatar_url, display_name, status, kratos_id, created_at, updated_at

          ...
          {screens.length > 0 && (
            <section>
              ...
              <div className="grid grid-cols-2 gap-3">
                {screens.map((source) => (
                  <SourceCard key={source.id} source={source} onSelect={startScreenShareWithSource} />
                ))}
              </div>
            </section>
          )}
...
function SourceCard({ source, onSelect }: {
  ...
  return (
    <button
      ...
      className="group flex flex-col rounded-lg border-2 border-transparent hover:border-sol-sage bg-sol-bg-elevated overflow-hidden transition-colors"
    >

...
local username =
  if std.objectHas(claims, 'nickname') && std.length(claims.nickname) > 0 then
    std.strReplace(std.strReplace(claims.nickname, '-', '_'), '.', '_')
  else if std.objectHas(claims, 'preferred_username') && std.length(claims.preferred_username) > 0 then
